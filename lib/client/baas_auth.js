                    ...
                    $http.get('https://graph.facebook.com/me?access_token=' + token)
                        .success(function (data) {
                            var user = {
                                ...
                                name: data.name,
                                f_name: data.name.split(' ').slice(0, 1).join(' '),
                                l_name: data.name.split(' ').slice(-1).join(' '),
                                role: role
                            };
    ...
    baasAuth.factory(
        ...
        [
            ...
            function ($http, Auth, settings, baasRequest, Alert) {
                ...
                function getReqToken(cb) {
                    baasRequest('GET', 'auth/twitter/request-token', cb);
                }

                function getAccessToken(secret, token, verifier, cb) {
                    baasRequest('POST', 'auth/twitter/access-token', {secret: secret, token: token, verifier: verifier}, cb);
                }

                function login(token, secret, role) {
                    // remove header as it can be sent over to fb call
                    delete $http.defaults.headers.common.authorization;
                    delete $http.defaults.headers.common['X-SS-BAAS-INSTALL-ID'];
                    ...
                    $http.get('https://api.twitter.com/user?access_token=' + token)
                        .success(function (data) {
                            var user = {id: data.id, username: data.email, name: data.name, role: role, twitterAuth: {token: token, secret: secret}};
                            baasRequest(
                                'post',
                                'auth/twitter/token',
                                {token: token, expiresIn: expiresIn, model: user},
                ...
                function setAuthToken(secret, token, verifier, role) {
                    busy = true;

                    getAccessToken(secret, token, verifier, function (err, data) {
                        if (err) {
                            Alert.error("Error: ", err);
                        } else {
                            console.log('** access token: ', data);
                            //login(token, secret, role);
                            //data //{token: oauth_token, secret: oauth_token_secret}
                        }
                    });
                }
