        ...
        [
            ...
            function ($window, $location, storage, $rootScope, $q, baasRequest, Alert) {
                ...
                return {
                    ...
                    register: function (user, success, isNoRedirect, cb) {
                        var self = this;
                        if (!cb) {
                            if (typeof isNoRedirect === 'function') {
                                cb = isNoRedirect;
                                isNoRedirect = false;
                        ...
                        baasRequest('post', 'auth/register', {model: user, loginOnSuccess: success}, function (err, data) {
                            if (err) {
                                ...
                                setUser(data);
                                self.setAuthToken('#token_type=bearer&access_token=' + data.token, isNoRedirect);
                                cb(null, data);
                            }
                        });
                    },
                    ...
                    resetPassword: function (id, token, newPwd) {
                        var deferred, model;
                        deferred = $q.defer();
                        model = {id: id, token: token, password: newPwd};

                        baasRequest('post', 'auth/reset-password', {model: model}, function (err) {
                            if (err) {
                                deferred.reject(err);
                            } else {
                                deferred.resolve();
                    ...
                    setAuthToken: function (locationHash, doNotChangeLocation) {
                        ...
                        if (params.err) {
                            ...
                            this.getUserInfo()
                                .then(function (user) {
                                    ...
                                    $rootScope.$broadcast('signed-in', user);
    ...
    baasAuth.directive(
        ...
        [
            ...
            function ($location, $rootScope, Auth, settings, Alert) {
                var port = parseInt(($location.port() || 80), 10);
                var host = $location.protocol() + '://' + $location.host() + ($location.port() && (port === 80 || port === 443)  ? '' : ':' + $location.port());
                ...
                return {
                    ...
                    template: '<div>' +
                        ...
                        '</form>' +
                        '<iframe id="sim-login-frame" name="baasLoginFrame" style="width: 0; height: 0; border: none; display: none;"></iframe>' +
                        '</div>',
                    transclude: true,
                    scope: {},
                    link: function (scope, element, attrs) {
                        var frm     = element.parent().find('form'),
                            ...
                            iframe  = angular.element(document.getElementById('sim-login-frame')),
                            i       = 0,
                            submitBtn,
                            pwdElem,
                            rememberMe,
                        ...
                        iframe.bind('load', function () {
                            ...
                            if (content && content !== "") {
                                ...
                                $rootScope.$apply(function () {
                                    Auth.setAuthToken(content, attrs.hasOwnProperty('doNotChangeLocation')); //(rememberMe && rememberMe[0].checked)
