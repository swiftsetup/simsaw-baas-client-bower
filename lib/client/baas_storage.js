...
define(['angular', 'simsaw/utils'], function (angular, utils) {
    ...
    function bindFileChange(baasRequest, scope, user, fileInput, data, Alert) {
        fileInput.bind('change', function () {
            var self = this,
                fType = self.files[0].type;

            // problem with file.type for csv on win machines.
            if(navigator.platform === "Win32" && scope.accept.indexOf('text/csv') > -1 && _.isEmpty(self.files[0].type)) {
                fType = 'text/csv';
            }

            if (validateFileExt(scope, fType, Alert)) {
