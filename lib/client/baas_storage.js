...
define(['angular', 'simsaw/utils'], function (angular, utils) {
    ...
    var _alert,
        ...
        fileUploadTemplate = '<span class="btn btn-default fileinput-button"> ' +
            ...
            '<p data-ng-repeat="upload in uploads | filter: {uploading: true}">' +
                '<progressbar id="uploadprogress" class="progress-bar progress-bar-success" min="0" max="100" value="upload.progress" type="success">{{upload.progress}}</progressbar>' +
        ...
        fileUploadDDTemplate = '<div ng-transclude>' +
                ...
                '<p data-ng-repeat="upload in uploads | filter: {uploading: true}">' +
                    '<progressbar class="progress-bar progress-bar-success" min="0" max="100" value="upload.progress" type="success">{{upload.progress}}</progressbar>' +
                    '&nbsp;<small>{{ upload.fileName }}</small>' +
                '</p>' +
            '</div>';


    function fileUpload(scope, elem, attrs, baasAuth, baasRequest) {
        ...
        getMediaData(baasRequest, scope, function (data) {
            ...
            if (testCompat().progress && testCompat().formData) {
                ...
                baasAuth.getUserInfo()
                    .then(function (user) {
                        bindFileChange(baasRequest, scope, user, fileInput, data);
                    });
    ...
    function validateFileExt(scope, fileType) {
        ...
        return fileType && types.indexOf(fileType) > -1
    }
