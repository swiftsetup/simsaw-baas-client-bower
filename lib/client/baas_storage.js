...
define(['angular', 'simsaw/utils'], function (angular, utils) {
    ...
    var _alert,
        ...
        fileUploadTemplate = '<span class="btn btn-default fileinput-button"> ' +
            ...
            ' <span>Upload</span>' +
            '<input type="file" name="files[]" multiple accept="{{accept}}"></span><br>' +
            ...
            '</p>',
        fileUploadDDTemplate = '<div class="file-upload-drop-zone"></div><br>' +
    ...
    function validateFileExt(scope, fileType) {
        if (fileType && (scope.accept || scope.accept.split(',').indexOf(fileType) > -1)) {
            return true;
        } else {
