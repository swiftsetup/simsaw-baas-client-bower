...
define(['angular', 'simsaw/utils'], function (angular, utils) {
    ...
    var _alert,
        ...
        fileUploadTemplate = '<span class="btn btn-default fileinput-button"> ' +
                ...
                '<span>Upload</span>' +
                '<span><input type="file" name="files[]" multiple"{{accept}}" capture="camera"/></span>' +
            '</span>' +
            '<br>' +
            ...
            '<p data-ng-repeat="upload in uploads | filter: {uploading: true}">' +
                '<progress id="uploadprogress" min="0" max="100" value="upload.progress" type="success">{{upload.progress}}</progress>' +
        ...
        fileUploadDDTemplate = '<div ng-transclude>' +
                ...
                '<p data-ng-repeat="upload in uploads | filter: {uploading: true}">' +
                    '<progress min="0" max="100" value="upload.progress" type="success">{{upload.progress}}</progress>' +
                    '&nbsp;<small>{{ upload.fileName }}</small>' +
                '</p>' +
            '</div>';
